...
<div class="reveal large _table-modal" id="add-modal" data-reveal>
    <h3 class="_table-modal-title">Add a new face shape</h3>
    <button class="close-button _table-modal-close" data-close aria-label="Close modal" type="button">
        <span aria-hidden="true">&times;</span>
    </button>

    <form action="<?= 'face_shapes.php?page=' . $page ?>" method="POST">
        <input type="hidden" name="token" value="<?=$token?>">
        <div class="grid-container">
            <div class="grid-x">
                <div class="cell account-field-cell" id="faceShape-shapeName">
                    <label class="account-field">shape_name<span class="account-required">*</span>
                        <input name="add_shapeName" type="text" placeholder="shapeName" required class="account-input" id="selected-shapeName-add"
                               maxlength="32">
                    </label>
                </div>
...
<div class="reveal large _table-modal" id="edit-modal" data-reveal>
    <h3 class="_table-modal-title">Edit a face shape</h3>
    <button class="close-button _table-modal-close" data-close aria-label="Close modal" type="button">
        <span aria-hidden="true">&times;</span>
    </button>
    <form action="<?= 'face_shapes.php?page=' . $page ?>" method="POST" id="edit-form">
        <input type="hidden" name="token" value="<?=$token?>">
        ...
        <div class="grid-container">
            <div class="grid-x">
                ...
                <div class="cell account-field-cell" id="faceShape-shapeName">
                    <label class="account-field">shape_name<span class="account-required">*</span>
                        <input type="text" placeholder="shape_name" name="put_faceShape_shapeName" required class="account-input" id="selected-shapeName-edit"
                               maxlength="512">
                    </label>
                </div>
...
<div class="reveal large _table-modal" id="delete-modal" data-reveal>
    <h3 class="_table-modal-title" id="delete-face_shape">Confirm delete face shape</h3>
    <form method="POST" action="<?= 'face_shapes.php?page=' . $page ?>">
        <input type="hidden" name="token" value="<?=$token?>">
            ...
            <div class="grid-x">
                <table class="_table-modal-delete">
                    ...
                    <tr class="_table-modal-delete-row">
                        ...
                        <td id="selected-id-delete" class="_table-modal-delete-val">faceShape_id</td>
                    </tr>
                    <tr class="_table-modal-delete-row">
                        <td class="_table-modal-delete-prop">shape_name:</td>
                        <td id="selected-shapeName-delete" class="_table-modal-delete-val">shape_name</td>
                    </tr>
    ...
    <main class="main">
        <div class="grid-x _tables-grid">
            <div class="cell small-12 large-11 large-offset-4 _tables">
                <h2 class="_tables-title">Face Shapes</h2>
                <form
                        action="<?= 'face_shapes.php?page='. $page ?>"
                        method="POST"
                ...
                <table class="_resource-table _faceShapes-table">
                    <thead>
                    <tr>
                        <th>id</th>
                        <th>shape_name</th>
                    ...
                    for ($i = 0; $i < count($faceShapes); $i++) { $faceShape = $faceShapes[$i]; ?>
                        <tr class="_tables-row">
                            ...
                            <td class="_tables-cell date_created"><?= date('F jS, Y h:i:s', strtotime($faceShape->dateCreated)) ?></td>
                            <td class="_tables-cell date_modified"><?= isset($faceShape->dateModified) ? date('F jS, Y h:i:s', strtotime($faceShape->dateModified)) : 'Never' ?></td>
                        </tr>
                    <?php } ?>
                    </tbody>
                </table>
