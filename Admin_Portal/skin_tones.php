...
/*if ($_POST && Utils::verifyCSRFToken()) {
    if (isset($_POST['_method'])) {
        ...
        $alert = $skinTone->handleSubmit();
    }
}

if (isset($_COOKIE["auth"])) {
    $p = Utils::paginateResource($skinTone, 'skinTones', ITEMS_PER_PAGE, $currentBaseUrl);
    $skinTones = $p['resources'];
    $count = $p['count'];
    $page = $p['page'];
...
<div class="reveal large _table-modal" id="add-modal" data-reveal>
    <h3 class="_table-modal-title">Add a new skin tone</h3>
    ...
    <form action="<?= 'skin_tones.php?page=' . $page ?>" method="POST">
        ...
        <div class="grid-container">
            <div class="grid-x">
                <div class="cell account-field-cell" id="skinTone-skinToneName">
                    <label class="account-field">skin_tone<span class="account-required">*</span>
                        <input name="add_skinToneName" type="text" placeholder="skinToneName" required class="account-input" id="selected-skinToneName-add"
                               maxlength="32">
                    </label>
                </div>
...
<div class="reveal large _table-modal" id="edit-modal" data-reveal>
    <h3 class="_table-modal-title">Edit a skin tone</h3>
    <button class="close-button _table-modal-close" data-close aria-label="Close modal" type="button">
        <span aria-hidden="true">&times;</span>
    </button>
    <form action="<?= 'skin_tones.php?page=' . $page ?>" method="POST" id="edit-form">
        ...
        <div class="grid-container">
            <div class="grid-x">
                ...
                <div class="cell account-field-cell" id="skinTone-skinToneName">
                    <label class="account-field">skin_tone<span class="account-required">*</span>
                        <input type="text" placeholder="skin_tone" name="put_hairLength_skinToneName" required class="account-input" id="selected-skinToneName-edit"
                               maxlength="512">
                    </label>
                </div>
            ...
            <div class="grid-x">
                <table class="_table-modal-delete">
                    ...
                    <tr class="_table-modal-delete-row">
                        ...
                        <td id="selected-id-delete" class="_table-modal-delete-val">skinTone_id</td>
                    </tr>
                    <tr class="_table-modal-delete-row">
                        <td class="_table-modal-delete-prop">skin_tone:</td>
                        <td id="selected-skinToneName-delete" class="_table-modal-delete-val">skin_tone</td>
                    </tr>
        ...
        <div class="grid-x _tables-grid">
            <div class="cell small-12 large-11 large-offset-4 _tables">
                ...
                <table class="_resource-table _skinTones-table">
                    ...
                    <tbody>
                    <?php
                    for ($i = 0; $i < count($skinTones); $i++) { $skinTone = $skinTones[$i]; ?>
                        <tr class="_tables-row">
                            <td class="_tables-cell id"><?= $skinTone->id ?></td>
                            <td class="_tables-cell skinToneName"><?= $skinTone->skinToneName ?></td>
                            <td class="_tables-cell date_created"><?= Utils::prettyPrintDateTime($skinTone->dateCreated) ?></td>
                            <td class="_tables-cell date_modified"><?= Utils::prettyPrintDateTime($skinTone->dateModified) ?></td>
                        </tr>
                    <?php } ?>
                    </tbody>
                </table>
